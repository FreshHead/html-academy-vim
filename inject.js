...
(function() {
    ...
    function activateVimMode() {
        ...
        editorElements.forEach(editorElement => {
            ...
            if (activatedCount === 0) {
                ace.config.set('basePath', 'https://unpkg.com/ace-builds@1.15.2/src-noconflict/');
            }
        ...
        if (activatedCount > 0) {
            console.log(`[Vim Mode] ✓ Activated Vim mode for ${activatedCount} editor(s)`);
            
            // Входим в insert mode для всех редакторов
            setTimeout(function() {
                enterInsertModeAll();
                showNotification(`✓ Vim mode активирован для ${editors.size} редактора(ов)!`);
            }, 200);
    ...
    function setupModeSync(editor) {
        ...
        const originalHandleKeyboard = editor.keyBinding.onCommandKey.bind(editor.keyBinding);
    ...
    function enterInsertModeAll() {
        ...
        if (successCount > 0) {
            currentMode = 'insert';
            console.log(`[Vim Mode] ✓ Entered INSERT mode for ${successCount} editor(s)`);
        }
    }
